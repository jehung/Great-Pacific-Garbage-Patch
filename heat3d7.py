...
def u(x,y,t):
    ...
    #ans = -0.1263*(cos(w*t)+sin(w*t))*sin(pi*p*x/200)*sin(q*pi*y/200)
    #ans = (-10/sqrt(2*pi*s))*exp(-(x**2+y**2)/(2.0*s)) #+ 0.1263*(cos(w*t)+sin(w*t))*sin(pi*p*x/200)*sin(q*pi*y/200)
    return ans
...
def animate(i):
    ...
    k += 1
    ax1.clear()
    #ax1.scatter(X,Y,Z,final, cmap = plt.hot())
...
def RandomWalk(N=10000):
    ...
    trend = np.random.uniform(-0.05, 0.05, N)
    all = np.cumsum(w+trend)+10
    for i in range(N):
        if all[i] >= 0: 
            pass
        else:
...
#PlotEndpoints(W=10000, N=10000, d=1)    
'''

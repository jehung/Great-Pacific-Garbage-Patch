...
plt.grid(True)
plt.title("Meridional velocity \n Positive number indicates northern direction")
colorbar(cs)
plt.show()
...
# do scale = 3000, 5000, 10000, 
S = [3000, 5000, 10000]

for s in S:
    plt.figure(s, figsize=(8, 8))
    plt.subplot()
    Q = plt.quiver(x, y, u, v, units='width', scale =10000) 
    plt.show()


    plt.subplot()
    Q = plt.quiver(x[::100], y[::100], u[::100], v[::100], units='width') 
    plt.title('OFES-NCEP surfac currents' + str(s))
    plt.show()
